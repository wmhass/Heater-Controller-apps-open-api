# Generated by Django 2.2 on 2019-10-07 17:01

from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='MqttAccount',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('username', models.CharField(max_length=40)),
                ('pw', models.CharField(max_length=100)),
                ('superuser', models.PositiveSmallIntegerField(default=0)),
            ],
        ),
        migrations.RunSQL("""
            ALTER TABLE
              mqttauthorization_mqttaccount
            ADD
              CONSTRAINT superuser CHECK (superuser = 0 OR superuser = 1)
        """),
        migrations.RunSQL("""
            INSERT INTO mqttauthorization_mqttaccount(username, pw, superuser)
            VALUES
            ('admin', 'PBKDF2$sha256$10000$59CmkTYlA/+4zy9m$t0WvH7vVotZp3wJVx+ZFMgADzIq7W5TD', 0)
        """),
        migrations.RunSQL("""
            INSERT INTO mqttauthorization_mqttaccount(username, pw, superuser)
            VALUES
            ('admin2', 'PBKDF2$sha256$10000$59CmkTYlA/+4zy9m$t0WvH7vVotZp3wJVx+ZFMgADzIq7W5TD', 0)
        """),
        migrations.CreateModel(
            name='MqttAcl',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('username', models.CharField(max_length=40)),
                ('topic', models.CharField(max_length=100)),
                ('rw', models.PositiveSmallIntegerField(default=1)),
            ],
        ),
        migrations.RunSQL("""
            ALTER TABLE
              mqttauthorization_mqttacl
            ADD
              CONSTRAINT rw CHECK (rw >= 1 AND rw <= 4)
        """),
        migrations.RunSQL("""
            INSERT INTO mqttauthorization_mqttacl(username, topic, rw)
            VALUES
            ('admin', 'user/admin', 2)
        """),
        migrations.RunSQL("""
            INSERT INTO mqttauthorization_mqttacl(username, topic, rw)
            VALUES
            ('admin2', 'user/admin2', 2)
        """)
    ]
